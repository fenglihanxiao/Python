...
class Game:
    ...
    def run(self):
        ...
        while True:     # 构造反复执行的机制，刷新窗体，使窗体保持在屏幕上
            ...
            """ TODO 6.删除一架敌机的移动控制
            self.enemy.move()       # 调用敌机移动操作
            self.enemy.display()    # 敌机显示到窗体中
            """
            for enemy in self.enemys:   # TODO 7.加入所有敌机
                enemy.move()    # TODO 7.每驾敌机移动
                enemy.display() # TODO 7.每驾敌机加入窗体
    ...
    def model_init(self):
        ...
        """
        self.enemys = []    # TODO 2.定义用于保存敌机的空列表，保存多驾敌机
        for _ in range(5) : # TODO 3.循环产生5架敌机
            self.enemys.append(EnemyPlane(IMG_ENEMY,random.randint(0, Game.WINDOW_WIDTH - 100 ),200))   # TODO 5.向列表添加敌机，初始位置x为横向随机，纵向200
