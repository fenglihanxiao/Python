...
class PlayerPlane(Model):
    ...
    # 重写玩家飞机display方法
    def display(self, enemys):  # TODO 2.在显示飞机和子弹的同时，传入敌机列表
        ...
        for bullet in self.bullets:     # 循环飞机中的子弹列表
            ...
            else:   # TODO 3.1.如果子弹位置未超出屏幕范围
                bullet_rect = pygame.locals.Rect(bullet.x,bullet.y, 20, 29) # TODO 4.创建子弹矩形对象，传入x,y,width,height
                for enemy in enemys:    # TODO 3.2使用子弹与所有敌机进行碰撞检测
                    enemy_rect = pygame.locals.Rect(enemy.x, enemy.y, 100, 68)  # TODO 5.创建敌机矩形对象，传入x,y,width,height
                    # TODO 6. 调用碰撞检测方法，传入当前子弹对象，传入敌机对象，判断是否碰撞
                    if pygame.Rect.colliderect(bullet_rect, enemy_rect):    # TODO 7.如果碰撞
                        print("相撞了")
...
class Game:
    ...
    def run(self):
        ...
        while True:     # 构造反复执行的机制，刷新窗体，使窗体保持在屏幕上
            ...
            self.player.display(self.enemys)  # TODO 1.飞机加入窗体时，传入敌机列表实参self.enemys # 玩家飞机加入窗体中
