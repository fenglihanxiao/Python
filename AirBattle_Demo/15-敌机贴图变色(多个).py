...
class EnemyPlane(Model):
    # 覆盖init方法
    def __init__(self):
        self.img = pygame.image.load(IMG_ENEMYS[random.randint(0,6)])   # TODO 3.设置图片路径随机从元组中获取
        """ TODO 4.删除原始定义格式
        self.img = pygame.image.load(IMG_ENEMY) # 使用常量
        """
    ...
    def move(self):
        ...
        if self.y > Game.WINDOW_HEIGHT: # 如果超出屏幕
            self.img = pygame.image.load(IMG_ENEMYS[random.randint(0, 6)]) # TODO 5.修改敌机到达底部后返回的图片随机生成，同初始化策略
