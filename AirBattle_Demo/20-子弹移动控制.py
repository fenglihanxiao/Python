...
class PlayerPlane(Model):
    ...
    def display(self):
        ...
        remove_bullets = [] # TODO 5.定义被删除的子弹列表
        for bullet in self.bullets:     # 循环飞机中的子弹列表
            bullet.move()       # TODO 3.调用子弹的移动操作
            bullet.display()    # 调用子弹加入窗体的操作
            # TODO 4.优化子弹存储队列，将超出屏幕的子弹删除
            if self.y < -29:    # TODO 6.如果子弹位置超出屏幕
                remove_bullets.append(bullet)   # TODO 7.将要删除的子弹加入到待删除列表

        for bullet in remove_bullets:   # TODO 8.循环删除子弹列表
            self.bullets.remove(bullet) # TODO 9.从原始子弹列表中删除要删除的子弹
