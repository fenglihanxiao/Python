...
class Game:
    ...
    def event_init(self):
        for event in pygame.event.get():    # 获取当前发生的所有事件
            ...
            if event.type == pygame.locals.MOUSEMOTION :    # TODO 1.设置监听鼠标移动事件
                pos = pygame.mouse.get_pos()    # TODO 2.获取鼠标位置坐标，结果是元组
                self.player.x = pos[0] - 120/2      # TODO 3.设置飞机中心位置在鼠标位置(x)  横坐标 - 飞机宽度1半
                self.player.y = pos[1] - 78/2 + 5   # TODO 3.设置飞机中心位置在鼠标位置(y)  纵坐标 - 飞机高度1半 +-微调数据
